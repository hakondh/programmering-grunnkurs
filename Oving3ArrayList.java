...
class OppgaveOversikt {
	...
	public boolean regNyStudent (String navn) {
		for (int i=0; i<studenter.size(); i++) {
			if (studenter.get(i).getNavn().compareTo(navn)==0) {
				return false;
			}
		}
	...
	public int finnAntOppgaver (String navn){
		String navnet = "";
		for (int i=0; i < studenter.size(); i++ ) {
			navnet=studenter.get(i).getNavn();
			if (navnet.compareTo(navn)==0) {
	...
	public boolean økAntOppg (String navn, int økning) {
		for (int i=0; i < studenter.size(); i++) {
			if (studenter.get(i).getNavn().compareTo(navn)==0) {
				int antOppgNå=studenter.get(i).getAntOppg();
				studenter.get(i).setAntOppg(antOppgNå+økning);
				return true;
			}
		}
		return false;
	...
	public String[] finnAlleNavn () {
		...
		for (int i=0; i < studenter.size(); i++) {
			if (studenter.get(i)==null) continue;
	...
	public String toString () {
		...
		for (int i=0; i < studenter.size(); i++) {
			if (studenter.get(i)==null) continue;
			studentinfo+="Navn: " + studenter.get(i).getNavn() + ", antall oppgaver løst: " + studenter.get(i).getAntOppg() + "\n";
		}
